<template>
  <div>
    <div class="qg-heading"><h2>白名单</h2></div>
    ...
    <div v-if="data" class="qg-layout">
      <div class="qg-side">
        ...
        <div v-for="g in data.groups" :key="g.id" class="qg-group-row">
          ...
          <label class="qg-apply" title="开启后本群直接使用全局白名单，隐藏本群独立白名单">
            <input type="checkbox" :checked="applyValue(g)" @change="toggleApply(g, $event)" /> 应用全局白名单
          </label>
        </div>
      </div>
      ...
      <div class="qg-card" style="margin:0">
        <div class="qg-heading">
          <h3>{{ currentScope === '' ? '全局白名单' : '群 ' + currentScope + ' 白名单' }}</h3>
        ...
        <div v-if="currentScope && isApplyingGlobal" class="qg-muted">
          该群已开启「应用全局白名单」，将直接使用全局白名单，本群独立白名单未启用。如需启用，请关闭左侧该群的「应用全局白名单」开关。
        </div>

        <template v-else>
          <p class="qg-muted">白名单用户默认豁免被加入黑名单；可在下方单独开启是否豁免被举报 / 入群审核 / 违禁词。</p>
          <div class="qg-add">
            <input class="qg-input" v-model="newWhitelistUserId" placeholder="QQ 号，多个用逗号/空格/换行分隔" />
            <button class="qg-btn primary" @click="addWhitelist">批量添加</button>
          </div>

          <table v-if="entries.length" class="qg-table" style="margin-top:10px">
            <thead><tr><th>QQ</th><th>豁免被举报</th><th>豁免入群审核</th><th>豁免违禁词</th><th>时间</th><th>操作</th></tr></thead>
            <tbody>
              <tr v-for="b in entries" :key="b.id">
                ...
                <td>
                  ...
                  <button class="qg-btn sm danger" @click="removeWhitelist(b.userId)">移除</button>
                </td>
              </tr>
            </tbody>
          </table>
          <div v-else class="qg-muted" style="margin-top:10px">暂无白名单记录</div>
        </template>
      </div>
    </div>
...
const entries = computed(() => (data.value?.whitelist || []).filter((b: any) => b.groupId === currentScope.value))

function applyValue(g: any) {
  const global = data.value?.global || {}
  const c = g?.config || {}
  return c.applyGlobalWhitelist !== undefined ? !!c.applyGlobalWhitelist : (global.applyGlobalWhitelist !== false)
}
...
async function toggleApply(g: any, e: Event) {
  ...
  await mutate('setGroup', { groupId: g.groupId, patch: { applyGlobalWhitelist: checked } })
}

async function addWhitelist() {
  const users = split(newWhitelistUserId.value)
  if (!users.length) return
  await mutate('whitelist.add', { users, groupId: currentScope.value })
  toast.success(`已添加 ${users.length} 个白名单`)
  newWhitelistUserId.value = ''
}

async function removeWhitelist(userId: string) {
  await mutate('whitelist.remove', { userId, groupId: currentScope.value })
  toast.info(`已移除 ${userId}`)
}

async function moveToGlobal(userId: string) {
  await mutate('whitelist.moveGlobal', { userId, groupId: currentScope.value })
}

async function moveAllToGlobal() {
  const users = entries.value.map((b: any) => b.userId)
  if (!users.length) return
  await mutate('whitelist.moveGlobal', { users, groupId: currentScope.value })
}

async function toggleFlag(b: any, key: string, e: Event) {
  const checked = (e.target as HTMLInputElement).checked
  await mutate('whitelist.update', { userId: b.userId, groupId: b.groupId, patch: { [key]: checked } })
}
</script>