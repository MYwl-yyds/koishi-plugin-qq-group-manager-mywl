...
class AiService {
    ...
    // 由上层流程降级到「默认操作」处理
    async reviewJoin(config, content) {
        try {
            // 固定提示词 + 用户自定义提示词自动合并（用户未填时仅使用固定内容）
            const custom = String(config.prompts?.joinReview || '').trim();
            const system = custom ? `${constants_1.JOIN_PROMPT_FIXED}\n\n【本群自定义审核要求】\n${custom}` : constants_1.JOIN_PROMPT_FIXED;
            ...
            this.log.warn('AI 入群审核失败，判定失效', e.message);
            return null;
        }
    }
    // 举报审核：返回结构化结论，失败降级返回不违规
    async reviewReport(config, content) {
        try {
            const system = config.prompts?.reportReview || constants_1.DEFAULT_REPORT_PROMPT;
            ...
            this.log.warn('AI 举报审核失败，降级不违规', e.message);
