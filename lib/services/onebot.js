...
class OneBotService {
    constructor(ctx, framework = 'auto') {
        // 群资料短时缓存，避免违禁词等高频通知重复调用 get_group_info
        this.groupProfileCache = new Map();
        this.ctx = ctx;
    ...
    async invoke(session, action, params, camelArgs) {
        const s = session;
    ...
    // 禁言（minutes 为分钟）
    async mute(session, userId, minutes) {
        try {
            const groupId = (0, utils_1.idOf)(session.guildId);
            const duration = Math.round(minutes * 60);
            await this.invoke(session, 'set_group_ban', { group_id: groupId, user_id: userId, duration }, [groupId, userId, duration]);
        }
        catch (e) {
    ...
    async unmute(session, userId) {
        try {
            const groupId = (0, utils_1.idOf)(session.guildId);
            await this.invoke(session, 'set_group_ban', { group_id: groupId, user_id: userId, duration: 0 }, [groupId, userId, 0]);
        }
        catch (e) {
    ...
    // 全体禁言 / 全体解禁
    async setWholeBan(session, enable) {
        try {
            const groupId = (0, utils_1.idOf)(session.guildId);
            await this.invoke(session, 'set_group_whole_ban', { group_id: groupId, enable }, [groupId, enable]);
        }
        catch (e) {
    ...
    async kick(session, userId) {
        try {
            const groupId = (0, utils_1.idOf)(session.guildId);
            await this.invoke(session, 'set_group_kick', { group_id: groupId, user_id: userId }, [groupId, userId]);
        }
        catch (e) {
    ...
    async leaveGroup(session) {
        try {
            const groupId = (0, utils_1.idOf)(session.guildId);
            await this.invoke(session, 'set_group_leave', { group_id: groupId }, [groupId]);
        }
        catch (e) {
    ...
    async handleJoinRequest(session, flag, subType, approve, reason) {
        try {
            await this.invoke(session, 'set_group_add_request', { flag, sub_type: subType, approve, reason: reason ?? '' }, [flag, subType, approve, reason ?? '']);
        }
        catch (e) {
    ...
    async handleFriendRequest(session, flag, approve, remark) {
        try {
            await this.invoke(session, 'set_friend_add_request', { flag, approve, remark: remark ?? '' }, [flag, approve, remark ?? '']);
        }
        catch (e) {
    ...
    async setEssence(session, messageId) {
        try {
            await this.invoke(session, 'set_essence_msg', { message_id: messageId }, [messageId]);
        }
        catch (e) {
    ...
    async deleteEssence(session, messageId) {
        try {
            await this.invoke(session, 'delete_essence_msg', { message_id: messageId }, [messageId]);
        }
        catch (e) {
    ...
    async setTitle(session, userId, title) {
        try {
            const groupId = (0, utils_1.idOf)(session.guildId);
            // 空标题表示清除头衔，duration 使用 -1 以兼容 napcat/llbot
            await this.invoke(session, 'set_group_special_title', { group_id: groupId, user_id: userId, special_title: title, duration: -1 }, [groupId, userId, title, -1]);
        }
        catch (e) {
    ...
    async recall(session, messageId) {
        try {
            await this.invoke(session, 'delete_msg', { message_id: messageId }, [messageId]);
        }
        catch (e) {
    ...
    async getMsg(session, messageId) {
        try {
            const res = await this.invoke(session, 'get_msg', { message_id: messageId }, [messageId]);
    ...
    async getStrangerInfo(session, userId) {
        try {
            const res = await this.invoke(session, 'get_stranger_info', { user_id: userId, no_cache: false }, [userId, false]);
            const data = res?.data ?? res ?? {};
            return {
    ...
    async getGroupMemberList(session, groupId) {
        try {
            const res = await this.invoke(session, 'get_group_member_list', { group_id: groupId }, [groupId]);
    ...
    async getGroupInfo(session, groupId) {
        try {
            const res = await this.invoke(session, 'get_group_info', { group_id: groupId }, [groupId]);
            return (res?.data ?? res ?? {});
        }
        catch (e) {
    ...
    async getGroupMemberInfo(session, groupId, userId) {
        try {
            const res = await this.invoke(session, 'get_group_member_info', { group_id: groupId, user_id: userId, no_cache: false }, [groupId, userId, false]);
            const data = res?.data ?? res ?? {};
            return {
    ...
    async getGroupProfile(session, groupId) {
        const gid = String(groupId || '');
        if (!gid)
            return { name: '', intro: '', avatar: '', memberCount: '' };
        ...
        const name = String(info.group_name ?? info.name ?? '').trim() || gid;
        const data = {
            name,
            intro,
        ...
        if (this.groupProfileCache.size > 200) {
            ...
            if (first)
                this.groupProfileCache.delete(first);
        }
        return data;
    }
    async sendGroup(session, groupId, content) {
        try {
            const res = await session.bot.sendMessage(groupId, content, groupId);
    ...
    async sendPrivate(session, userId, content) {
        try {
            const res = await session.bot.sendPrivateMessage(userId, content);
            const ids = Array.isArray(res) ? res : [res];
            return String(ids[0] ?? '');
        }
        catch (e) {
...
exports.OneBotService = OneBotService;
