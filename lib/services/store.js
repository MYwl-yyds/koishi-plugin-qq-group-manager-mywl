...
class Store {
    ...
    // ---------- 黑名单 ----------
    async blacklistAdd(userId, source = 'manual', groupId = '') {
        ...
        if (wlGlobal.length > 0 || wlGroup.length > 0)
            return;
    ...
    async blacklistRemove(userId, groupId = '') {
        const res = await this.db.remove('gm_blacklist', { userId, groupId });
        return res?.removed > 0 || res?.[0]?.removed > 0 || !!res;
    }
    async blacklistList(groupId) {
    ...
    async blacklistHas(userId, groupId = '', applyGlobal = true) {
        const query = applyGlobal ? { userId, groupId: '' } : { userId, groupId };
        const res = await this.db.get('gm_blacklist', query);
        return res.length > 0;
    }
    // 将某群的黑名单迁移到全局黑名单
    async blacklistMoveToGlobal(userId, groupId) {
    ...
    // ---------- 白名单 ----------
    async whitelistAdd(userId, groupId = '', flags = {}) {
    ...
    async whitelistUpdate(userId, groupId, patch) {
        await this.db.set('gm_whitelist', { userId, groupId }, patch);
    }
    async whitelistRemove(userId, groupId = '') {
        const res = await this.db.remove('gm_whitelist', { userId, groupId });
        return res?.removed > 0 || res?.[0]?.removed > 0 || !!res;
    }
    async whitelistList(groupId) {
        const query = groupId === undefined ? {} : { groupId };
        return this.db.get('gm_whitelist', query, { sort: { createdAt: 'desc' } });
    }
    // 获取某用户的白名单条目（applyGlobal 为 true 查全局，否则查该群）
    async whitelistEntry(userId, groupId = '', applyGlobal = true) {
        const scope = applyGlobal ? '' : groupId;
        const res = await this.db.get('gm_whitelist', { userId, groupId: scope });
        return res[0];
    }
    // 将某群的白名单迁移到全局白名单
    async whitelistMoveToGlobal(userId, groupId) {
    ...
    async permissionGroupByName(name) {
        const res = await this.db.get('gm_permission_group', { name });
        return res[0];
    }
    async permissionGroupCreate(data) {
        return (await this.db.create('gm_permission_group', data));
    }
    async permissionGroupUpdate(id, patch) {
        await this.db.set('gm_permission_group', { id }, patch);
    }
    async permissionGroupRemove(id) {
        await this.db.remove('gm_permission_group', { id });
    }
    // ---------- 群级配置 ----------
    async groupConfigGet(groupId) {
        const res = await this.db.get('gm_group_config', { groupId });
        return res[0];
    }
    async groupConfigSet(groupId, config) {
    ...
    // ---------- 入群申请 ----------
    async joinRequestCreate(data) {
        return (await this.db.create('gm_join_request', data));
    }
    async joinRequestByFlag(flag) {
        const res = await this.db.get('gm_join_request', { flag });
        return res[0];
    }
    async joinRequestUpdate(flag, patch) {
        await this.db.set('gm_join_request', { flag }, patch);
    }
    async joinRequestList(status) {
        const query = status ? { status } : {};
        return this.db.get('gm_join_request', query, { sort: { createdAt: 'desc' } });
    }
    async joinRequestRemove(flag) {
        await this.db.remove('gm_join_request', { flag });
    }
    async joinRequestCountRecent(userId, groupId, since) {
        const res = await this.db.get('gm_join_request', { userId, groupId, createdAt: { $gte: since } });
        return res.length;
    }
    // 清理过期申请（由定时器调用）
    async joinRequestCleanup() {
        await this.db.remove('gm_join_request', { status: 'pending', expireAt: { $lt: new Date() } });
    }
    async logAdd(entry) {
        await this.db.create('gm_log', { ...entry, createdAt: new Date() });
    ...
    async pruneLogs() {
        ...
        if (all.length <= Store.MAX_LOG_COUNT)
            return;
