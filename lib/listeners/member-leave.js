...
function apply(ctx, svc) {
    ctx.on('guild-member-removed', async (session) => {
        try {
            ...
            if (!cfg.autoBlacklist.enabled)
                return;
            const operatorId = (0, utils_1.idOf)(session.operatorId);
            const selfLeave = !operatorId || operatorId === userId;
            const nickname = await svc.onebot.resolveNickname(session, groupId, userId);
            if (selfLeave && !cfg.autoBlacklist.onSelfLeave)
                return;
            if (!selfLeave && !cfg.autoBlacklist.onKicked)
                return;
...
//# sourceMappingURL=member-leave.js.map