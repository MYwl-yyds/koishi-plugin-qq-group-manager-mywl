...
function apply(ctx, svc) {
    const forward = async (session, type, title, vars, flag) => {
        try {
            const cfg = await svc.settings.getGlobal();
            const fw = cfg.requestForward;
            if (!fw?.enabled || !fw?.targetId)
                return;
            const text = (0, utils_1.template)(fw.text || '【{title}通知】\n申请人：{nickname}({userId})\n申请内容：{comment}', { title, ...vars });
            let mid = '';
            if (fw.mode === 'private') {
            ...
            if (mid)
                (0, request_tracker_1.trackRequest)(mid, { flag, type, source: 'forward' });
    ...
    ctx.middleware(async (session, next) => {
        try {
            ...
            const operatorId = (0, utils_1.idOf)(session.userId);
            if (meta.type === 'friend') {
                // 备注：未填写则不附带备注
                ...
                await svc.log.audit(approve ? '通过加好友申请(转发)' : '拒绝加好友申请(转发)', { operatorId, operatorName: session.username || '', detail: meta.flag });
            ...
            await svc.log.audit(approve ? '通过入群申请(转发)' : '拒绝入群申请(转发)', {
                ...
                groupId: record?.groupId ?? (0, utils_1.idOf)(session.guildId),
                detail: reason,
            });
...
//# sourceMappingURL=request-forward.js.map