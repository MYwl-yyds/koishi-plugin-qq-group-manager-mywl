...
async function buildSnapshot(svc) {
    ...
    for (const day of days)
        trendMap[day] = { date: day, operation: 0, audit: 0, violation: 0, blacklist: 0 };
    for (const l of logs) {
        const day = formatDate(new Date(l.createdAt));
        if (trendMap[day] && trendMap[day][l.type] !== undefined)
            trendMap[day][l.type]++;
    }
    // 违规类型分布
    const violationDist = {};
...
// 处理 WebUI 变更请求
async function handleMutate(svc, action, data, operatorName = '') {
    // WebUI 操作统一附带操作者名称（auth 插件登录用户名）
    const log = {
        operation: (a, p = {}) => svc.log.operation(a, { ...p, operatorName }),
        audit: (a, p = {}) => svc.log.audit(a, { ...p, operatorName }),
        blacklist: (a, p = {}) => svc.log.blacklist(a, { ...p, operatorName }),
    };
    try {
        switch (action) {
            case 'setGlobal': {
                // 通知转发不能设置为群聊管理中已配置的群聊
                const fw = data?.requestForward;
                ...
                await svc.settings.setGlobal(data);
            ...
            case 'permission.import': {
                const bot = svc.ctx.bots.find((b) => b.internal || b.onebot) || svc.ctx.bots[0];
                if (!bot)
                    return { ok: false, error: '未找到 OneBot 协议机器人' };
                const members = await svc.onebot.getGroupMemberList({ bot }, data.groupId);
...
async function approveJoin(svc, flag, approve, reason, operatorName = '') {
    const record = await svc.store.joinRequestByFlag(flag);
    if (!record)
        return { ok: false, error: '未找到该入群申请记录' };
    const bot = svc.ctx.bots.find((b) => b.onebot ?? b.internal) || svc.ctx.bots[0];
    if (!bot)
        return { ok: false, error: '未找到 OneBot 协议机器人' };
    try {
        ...
        if (raw && fw !== 'auto') {
            ...
            await bot.internal.setGroupAddRequest(flag, record.subType || 'add', approve, reason);
        }
    }
    catch (e) {
        return { ok: false, error: `调用 OneBot 失败：${e.message}` };
    }
...
function applyWebUI(ctx, svc) {
    const consoleService = ctx.console;
    if (!consoleService)
        return;
    class GmData extends plugin_console_1.DataService {
        constructor(cctx) {
            super(cctx, 'qq-guanqun');
        }
        async get() {
    ...
    ctx.plugin(GmData);
    consoleService.addListener('qq-guanqun/mutate', async function (payload) {
