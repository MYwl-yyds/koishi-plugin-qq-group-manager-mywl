...
function apply(ctx, svc) {
    ctx.command('通过入群申请 <flag:text>', '同意一个入群申请')
        ...
        if (!await svc.permission.check(session, '入群审核'))
            return '你没有权限使用此命令';
        if (!flag)
            return '请提供入群申请的 flag（见审核通知）';
        ...
        if (record)
            await svc.store.joinRequestUpdate(flag, { status: 'approved' });
        await svc.log.audit('通过入群申请', { operatorId: (0, utils_1.idOf)(session.userId), targetId: record?.userId ?? '', groupId: record?.groupId ?? guild(session), detail: flag });
        return '已同意该入群申请';
    });
    ctx.command('拒绝入群申请 <flag:text> [reason:text]', '拒绝一个入群申请')
        .action(async ({ session }, flag, reason) => {
        if (!await svc.permission.check(session, '入群审核'))
            return '你没有权限使用此命令';
        if (!flag)
            return '请提供入群申请的 flag（见审核通知）';
        ...
        if (record)
            await svc.store.joinRequestUpdate(flag, { status: 'rejected' });
        await svc.log.audit('拒绝入群申请', { operatorId: (0, utils_1.idOf)(session.userId), targetId: record?.userId ?? '', groupId: record?.groupId ?? guild(session), detail: reason?.trim() || '' });
        return '已拒绝该入群申请';
    });
    ...
    ctx.command('通过加好友申请 <flag:text> [remark:text]', '同意加好友申请（仅超级管理员）')
        .action(async ({ session }, flag, remark) => {
        if (!await svc.permission.isSuperAdmin(session))
            return '仅超级管理员可操作';
        if (!flag)
            return '请提供好友申请的 flag（见通知）';
        try {
        ...
        await svc.log.audit('通过加好友申请', { operatorId: (0, utils_1.idOf)(session.userId), detail: flag });
        return '已同意该加好友申请';
    });
    ctx.command('拒绝加好友申请 <flag:text> [remark:text]', '拒绝加好友申请（仅超级管理员）')
        ...
        await svc.log.audit('拒绝加好友申请', { operatorId: (0, utils_1.idOf)(session.userId), detail: flag });
        return '已拒绝该加好友申请';
    });
}
