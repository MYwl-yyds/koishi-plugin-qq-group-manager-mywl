...
function apply(ctx, svc) {
    ctx.command('举报', '举报一条被引用的消息（需先引用/回复目标消息）')
        ...
        if (cfg.report.frequency?.enabled) {
            ...
            const count = await svc.store.logCountRecent('violation', (0, utils_1.idOf)(session.userId), guild(session), since);
        ...
        if (!quote)
            return '请先「引用或回复」要举报的消息，再发送举报命令';
        const messageId = (0, utils_1.idOf)(quote.id ?? quote.messageId);
        let target = (0, utils_1.idOf)(quote.userId);
        ...
        if (!target)
            return '无法获取被举报消息的发送者 QQ，请重新「引用」该消息后重试';
        // 白名单豁免被举报
        const wlReport = await svc.store.whitelistEntry(target, guild(session), cfg.applyGlobalWhitelist !== false);
        if (wlReport?.exemptReport)
            return '该用户已被豁免举报，无法对其发起举报';
        // 组装上下文（被引用消息 + 举报人附言）
        const context = `被举报消息（发送人 ${target}）：\n${content}\n\n举报人附言：\n${session.content || ''}`;
        // 调用 AI 审核（使用举报场景提示词）
        if (!cfg.ai.enabled || !cfg.ai.apiKey) {
            await svc.log.violation('举报（AI未启用）', { operatorId: (0, utils_1.idOf)(session.userId), operatorName: session.username || '', targetId: target, groupId: guild(session), detail: content.slice(0, 200) });
            return 'AI 接口未启用，无法自动判定举报内容。请联系管理员配置 AI 接口。';
        }
        const verdict = await svc.ai.reviewReport(cfg.ai, context);
        if (!verdict.violation) {
            await svc.log.violation('举报（未违规）', { operatorId: (0, utils_1.idOf)(session.userId), operatorName: session.username || '', targetId: target, groupId: guild(session), detail: content.slice(0, 200) });
            return '经 AI 审核，该消息暂未发现违规行为。';
        }
        // 根据违规程度映射惩罚
        const level = verdict.level || '轻度';
        const type = verdict.type || '其他';
        const punishment = cfg.report.levels.find((l) => l.level === level) || cfg.report.levels.find((l) => l.level === '中度');
        const gid = guild(session);
        const results = [`判断为「${type} / ${level}」违规`];
        if (punishment) {
            try {
                ...
                results.push(`惩罚执行失败：${e.message}`);
            }
        }
        await svc.log.violation('举报处理', {
            operatorId: (0, utils_1.idOf)(session.userId), operatorName: session.username || '',
            targetId: target,
            groupId: gid,
