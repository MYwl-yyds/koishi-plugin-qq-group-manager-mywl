...
import { ALL_COMMANDS } from '../services/permission'

function guild(session: Session): string {
  return idOf(session.guildId)
}

function parseGroupIds(text: string | undefined): string[] {
  if (!text || text.trim() === '' || text.trim() === '全部') return []
  return text.split(/[,，\s]+/).filter((x) => /^\d{5,}$/.test(x))
}

function parseGroupId(text: string | undefined): string {
  const m = String(text || '').match(/\d{5,}/)
  return m ? m[0] : ''
}

function normalizeRole(input: string | undefined): 'admin' | 'member' | 'owner' {
  const r = String(input || '').trim().toLowerCase()
  if (['admin', '管理员', '管理'].includes(r)) return 'admin'
  if (['owner', '群主', '所有人', '群所有人'].includes(r)) return 'owner'
  return 'member'
}
